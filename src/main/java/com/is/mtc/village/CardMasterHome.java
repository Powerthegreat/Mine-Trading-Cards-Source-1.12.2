...
*/public class CardMasterHome {/* extends StructureVillagePieces.Village {
	...
	public boolean addComponentParts(World world, Random random, StructureBoundingBox structureBB) {
		...
		if (this.biome == null) {
			this.biome = world.getBiome(new BlockPos((this.boundingBox.minX + this.boundingBox.maxX) / 2, 0, (this.boundingBox.minZ + this.boundingBox.maxZ) / 2));
		}

		// Set structureType based on biome so that the vanilla hooks will use it
		if (this.biome instanceof BiomeDesert) {
			this.structureType = 1;
		} else if (this.biome instanceof BiomeSavanna) {
			this.structureType = 2;
		} else if (this.biome instanceof BiomeTaiga) {
